...
class Deposit(Instrument):
    def __init__(self, valuationdate, quote, tenor, settledays, daycount, calendar, discurve,
        ...
        self.index, self.curve = None, None
...
class Swap(Instrument):  # single currency fix-float swap
    def __init__(self,
        ...
        self.index, self.forcurve, self.discurve, self.IBORIndex, self.QLSWAP = None, None, None, None, None
...
class BasisSwap(Instrument):  # single currency float-float basis swap
    def __init__(self,
        ...
        period = ql.Period(maturity)
        self.enddate = self.calendar.advance(self.startdate, period, businessday_convention, True)
        self.index, self.zLeg1forcurve, self.zLeg1forcurve, self.zLeg2forcurve, self.zdiscurve1, self.zdiscurve2, \
            self.IBORIndex1, self.IBORIndex2 = None, None, None, None, None, None, None, None
...
class OIS(Instrument):
    def __init__(self,
        ...
        self.index, self.zcurve, self.onindex, self.QLOIS = None, None, None, None
...
class CCBS(Instrument):  # cross currency basis swap, float-float (resettble)
    def __init__(self,
                 valuationdate,
                 quote,  # fixed leg rate
                 maturity,  # string for the tenor, e.g. '3M'
        ...
        super().__init__(valuationdate, quote, 'CCBS')
        self.maturity = maturity
        self.settledays = int(settledays)
        self.Leg1Daycount = Leg1Daycount
        self.Leg2Daycount = Leg2Daycount
    ...
    def impliedquote(self):
        ...
        Leg1BPS = self.Leg1BPS()
        bp = 1.e-4
        impliedquote = (NPV2 - NPV1) / Leg1BPS * bp
        impliedquote += self.quote
        return impliedquote
...
class CurveSet(object):
    ...
    def writezerostoExcel(self, wb):
        """
        write the zero curve to Excel, wb is workbook object by xlwings
        """
    ...
    def writeJacobiantoExcel(self, wb):
        """
        write the Jacobian matrix to Excel, wb is workbook object by xlwings
        """

        sht = wb.sheets['JacobianMatrix']
        i = 0
        for curveName in self.curveset.keys():
            labels = [curveName + tenor for tenor in self.curveset[curveName].tenors]
            sht.range((3 + i, 2)).options(transpose=True).value = labels
            sht.range((2, 3 + i)).value = labels
